...
describe("chat persistence", () => {
  ...
  it("ignores the removed legacy favorite flag", () => {
    const state = normalizeWorkspaceState({
      tasks: [{
        ...task("task-1"),
        messages: [{
    ...
    expect(state.tasks[0]?.messages[0]).not.toHaveProperty("favorite");
  });

  it("exports full task references and tool operations", () => {
